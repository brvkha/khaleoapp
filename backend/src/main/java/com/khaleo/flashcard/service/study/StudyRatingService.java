...
public class StudyRatingService {
    ...
    public RateCardResponse rateCard(UUID cardId, RateCardRequest request) {
            ...
            CardLearningState saved = cardLearningStateUpdateService.saveWithSingleRetry(
                    ...
                    () -> {
                        ...
                        SpacedRepetitionService.RatingOutcome outcome =
                                studySchedulerService.apply(current, request.rating(), now, userZone);
        ...
        SpacedRepetitionService.RatingOutcome againOutcome = studySchedulerService.apply(copyCardState(current), RatingGiven.AGAIN, now, userZone);
        SpacedRepetitionService.RatingOutcome hardOutcome = studySchedulerService.apply(copyCardState(current), RatingGiven.HARD, now, userZone);
        SpacedRepetitionService.RatingOutcome goodOutcome = studySchedulerService.apply(copyCardState(current), RatingGiven.GOOD, now, userZone);
        SpacedRepetitionService.RatingOutcome easyOutcome = studySchedulerService.apply(copyCardState(current), RatingGiven.EASY, now, userZone);
